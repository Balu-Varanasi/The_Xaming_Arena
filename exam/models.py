...
class Question(models.Model):
	...
	image = models.ImageField(_('image'),upload_to = "/static/image/", blank=True)
	date_entered = models.DateTimeField(_('today date'), default=datetime.datetime.now)
	class Meta:
		verbose_name = _('Question')
		verbose_name_plural = _('Questions')

	def __unicode__(self):
		return "%s %s %s %s %s %s %s"%(self.id,self.subject_code, self.question,self.A,self.B,self.C,self.D)
...
class AbstractAnswerClass(models.Model):
        """This is a Abstract class table which have details of the correct answers of every exam."""
	one = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	two = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	three = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	four = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	five = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	six = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	seven = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	eight = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	nine = models.CharField(max_length = 1, choices = ANSWER_CHOICES)
	ten = models.CharField(max_length = 1, choices = ANSWER_CHOICES)

	class Meta:
		abstract = True
	
	def __unicode__(self):
		return "%s %s %s %s %s %s %s %s %s %s" % (self.one, self.two, self.three, self.four, self.five, self.six, self.seven, self.eight, self.nine, self.ten)
